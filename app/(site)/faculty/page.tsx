...
export const metadata = { title: 'Our Faculty — Meet Your Trainers' };
        ...
        <div className="container-wrap grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {faculty.map((f, idx) => (
            <div key={f.name} className="card">
              ...
              <div className="mt-4 rounded-md bg-slate-50 px-3 py-2 text-xs text-slate-700 font-medium">
                🎯 {f.specialization}
              </div>
              <p className="mt-4 text-sm text-slate-600 leading-relaxed">{f.bio}</p>
            </div>
          ))}
        </div>
      </section>
