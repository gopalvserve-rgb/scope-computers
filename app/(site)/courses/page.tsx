...
import { getCourses, seedAllIfEmpty } from '@/lib/cms';
    ...
    <>
      <section className="bg-gradient-to-br from-brand-900 to-brand-700 text-white">
        <div className="container-wrap py-16 lg:py-20">
          ...
          <p className="mt-4 max-w-3xl text-brand-100 leading-relaxed">
            Short tracks, diploma programs, and government-approved certifications — matched to your career goals and taught by instructors with real industry experience.
          </p>
          ...
          {Object.entries(byCategory).map(([cat, list]) => (
            <div key={cat}>
              <div className="flex items-center gap-4">
                ...
                <span className="text-sm text-slate-500">{list.length} courses</span>
              </div>
              <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {list.map((course) => (
                  <div key={course.id} className="card flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      {course.description && <p className="mt-1 text-xs text-slate-600">{course.description}</p>}
                      <div className="mt-2 flex gap-2">
                        <Link href="/admission" className="text-xs font-semibold text-brand-700 hover:text-brand-800">Enroll →</Link>
                        <Link href="/syllabus" className="text-xs font-semibold text-slate-500 hover:text-brand-700">Syllabus →</Link>
                      </div>
                    </div>
                  </div>
                ))}
      ...
      <section className="section bg-slate-50">
        <div className="container-wrap max-w-2xl text-center">
          ...
          <p className="section-sub mx-auto">Our career counselors can help you pick the right course based on your background and goals.</p>
